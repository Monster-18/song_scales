...
class _Fv3State extends State<Fv3> {
  ...
  Future<List<ScaleModel>> get rows async{
    List<ScaleModel> list= await fv3.queryAllRows(songStart);
    if(list.isEmpty){
      return [];
    }else{
      return list;
...
class _CardsState extends State<Cards> {
  ...
  Future<void> deleteAlert(){
    FlatButton deleteButton = new FlatButton(
      onPressed: () async{
        show = false;
        await fv3.deleteData(widget.scale.id);
        Navigator.pop(context);
      },
      child: Text(
        'Delete',
