...
class _Fv2State extends State<Fv2> {
  ...
  Future<List<ScaleModel>> get rows async{
    List<ScaleModel> list= await fv2.queryAllRows(songStart);
    if(list.isEmpty){
      return [];
    }else{
      return list;
...
class _CardsState extends State<Cards> {
  ...
  Future<void> deleteAlert(){
    FlatButton deleteButton = new FlatButton(
      onPressed: () async{
        show = false;
        await fv2.deleteData(widget.scale.id);
        Navigator.pop(context);
      },
      child: Text(
        'Delete',
